...
// Get pooling layer according to engine.
template <typename Dtype>
shared_ptr<Layer<Dtype> > GetLSTMUnitLayer(const LayerParameter& param) {
	LSTMUnitParameter_Engine engine = param.lstmunit_param().engine();
	if (engine == LSTMUnitParameter_Engine_DEFAULT) {
		engine = LSTMUnitParameter_Engine_CAFFE;
#ifdef USE_CUDNN
		engine = LSTMUnitParameter_Engine_CUDNN;
#endif
	}
	if (engine == LSTMUnitParameter_Engine_CAFFE) {
		return shared_ptr<Layer<Dtype> >(new PoolingLayer<Dtype>(param));
#ifdef USE_CUDNN
	}
	else if (engine == LSTMUnitParameter_Engine_CUDNN) {
		if (param.top_size() > 1) {
			LOG(INFO) << "cuDNN does not support multiple tops. "
				<< "Using Caffe's own pooling layer.";
			return shared_ptr<Layer<Dtype> >(new PoolingLayer<Dtype>(param));
...
//REGISTER_LAYER_CREATOR(LSTMUnit, GetLSTMUnitLayer);
#ifdef WITH_PYTHON_LAYER
template <typename Dtype>
